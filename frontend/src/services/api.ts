...
export async function fetchMatchesByCompetition(code: string): Promise<MatchFixture[]> {
  try {
    ...
    console.warn('[Frontend API] Backend offline or unreachable. Using cached fixtures fallback.', error);
    return MOCK_FIXTURES.filter((f) => f.competitionCode === code);
  }
}
...
export async function fetchTeams(): Promise<Team[]> {
  try {
    ...
    console.warn('[Frontend API] Backend offline or unreachable. Using cached teams fallback.', error);
    return MOCK_TEAMS;
  }
}
